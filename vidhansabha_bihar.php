...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $encrypted_vote = encryptVote($vote);

    // Step 2: Save the encrypted vote
    $insert_vote_query = "INSERT INTO state_votes ( encrypted_vote) VALUES ( '$encrypted_vote')";
    ...
    if ($conn->query($insert_vote_query)) {
        ...
        if ($vote === "NOTA") {
            ...
            $conn->query("UPDATE state_party SET votes_count = votes_count + 1 WHERE party_name_english = '$vote'");
        }
...
$vote_results = $conn->query("SELECT party_name_english, votes_count FROM state_party ORDER BY votes_count DESC");
...
<body>
    <div class="evm-container">
        ...
        <ul class="candidate-container">
            ...
            $partyQuery = $conn->query("SELECT * FROM state_party");
            $candidateCounter = 1;
