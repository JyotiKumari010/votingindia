...
if (isset($_POST['add_party'])) {
    ...
    if ($uploadOk == 1) {
        ...
        if (move_uploaded_file($_FILES["party_symbol"]["tmp_name"], $target_file)) {
            ...
            $sql = "INSERT INTO party (party_symbol, party_name_english, party_name_urdu, party_name_local, background) 
...
if (isset($_GET['delete'])) {
    ...
    if ($conn->query($sql)) {
        header("Location: voting.php"); // Redirect after deletion
        exit();
    } else {
...
if (isset($_POST['update_party'])) {
    ...
    $sql = "UPDATE party SET $setClause WHERE id = $id";

    if ($conn->query($sql)) {
        header("Location: voting.php"); // Redirect after update
        exit();
    } else {
...
// Fetch all parties
$party = $conn->query("SELECT * FROM party");
?>
<!DOCTYPE html>
<html lang="en">
<head>
